...
model=Sequential()
model.add(ConvLSTM2D(1,3,activation='relu',input_shape=(n_seq, 1, n_steps, n_features)))
model.add(ConvLSTM2D(1,3,activation='relu'))
model.add(ConvLSTM2D(1,3,activation='relu'))
model.add(Flatten())
model.add(Dense(n_steps_out))
model.compile(optimizer='adam',loss='mse')

net=model.fit(x_train,y_train,epochs=2,validation_split=0.2)
NetPlot(net)
